...
// Write an 8-bit cmd
static esp_err_t SPI_WriteData(uint8_t data)//oled_write_cmd
{
...
static esp_err_t SPI_WriteData_command(uint8_t data)//oled_write_cmd
{
    uint32_t buf = data << 24; // In order to improve the transmission efficiency, it is recommended that the external incoming data is (uint32_t *) type data, do not use other type data.
    spi_trans_t trans = {0};
    trans.mosi = &buf;
...
static esp_err_t SPI_WriteData_data(uint8_t data)//oled_write_cmd
{
    uint32_t buf = data << 24; // In order to improve the transmission efficiency, it is recommended that the external incoming data is (uint32_t *) type data, do not use other type data.
    spi_trans_t trans = {0};
    trans.mosi = &buf;
...
static esp_err_t Lcd_WriteIndex(uint8_t data)//oled_write_cmd
{
    // oled_set_dc(0);
    SPI_WriteData_command(data);//SPI_WriteData(data);
    return ESP_OK;
}

static esp_err_t Lcd_WriteData(uint8_t data)//oled_write_cmd
{
    // oled_set_dc(1);
    SPI_WriteData_data(data);//SPI_WriteData(data);
    return ESP_OK;
}
...
void  LCD_WriteData_16Bit(unsigned int Data)
{
	...
	//oled_set_dc(1);//LCD_RS_SET_1;//rs=1;rs=1;
	SPI_WriteData_data(Data>>8); 	//??????8??????
	SPI_WriteData_data(Data); 			//??????8??????
...
void app_main(void)
{
    ...
    while (1) {
        oled_delay_ms(1000);
        Display_ASCII8X16(20,20,"Sun Yuhong",RED);
        // printf("...");
        ESP_LOGI(TAG, ".");
    }
}